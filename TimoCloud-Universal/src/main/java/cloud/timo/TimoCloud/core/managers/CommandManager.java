...
public class CommandManager {
    ...
    public void onCommand(String command, String ... args) {
        ...
        if (command.equalsIgnoreCase("removegroup")) {
            ...
            try {
                ...
                if (serverGroup != null) TimoCloudCore.getInstance().getServerManager().removeServerGroup(serverGroup);
        ...
        if (command.equalsIgnoreCase("addgroup")) {
            ...
            if (type.equalsIgnoreCase("server")) {
                ...
                ProxyGroup proxyGroup = new ProxyGroup(name, playersPerProxy, maxPlayers, keepFreeSlots, maxAmount, ram, null, isStatic, priority, Collections.singletonList("*"), baseName);
            ...
            if (group instanceof ServerGroup) {
                ...
                switch (key.toLowerCase()) {
                    ...
                    case "base":
                        ...
                        if (baseName.equalsIgnoreCase("none") || baseName.equalsIgnoreCase("dynamic")) baseName = null;
                ...
                switch (key.toLowerCase()) {
                    case "playersperproxy":
                        proxyGroup.setPlayersPerProxy(Integer.parseInt(value));
                        break;
                    case "maxplayers":
                        proxyGroup.setMaxPlayers(Integer.parseInt(value));
                        break;
                    case "keepfreeslots":
    ...
    private void displayGroup(ProxyGroup group) {
        send("  " + group.getName() +
                " (RAM: " + group.getRam() + "M" +
                ", Current-Online-Players: " + group.getOnlinePlayerAmount() +
                ", Total-Max-Players: " + group.getMaxPlayers() +
                ", Players-Per-Proxy: " + group.getPlayersPerProxy() +
    ...
    public void sendHelp() {
        ...
        send("  addgroup proxy <groupName (String)> <playersPerProxy (int)> <maxPlayers (int)> <keepFreeSlots (int)> <maxAmount (int)> <ram (int)> <static (boolean)> <priority (int)> <base (String), optional> - creates a proxy group. If no base is specified, one will get chosen dynamically.");
        ...
        send("  editgroup <name> <playersPerProxy (int) | maxPlayers (int) | keepFreeSlots (int) | maxAmount (int) | base (String) | ram (int) | static (boolean) | priority (int)> <value> - edits the give setting of a proxy group");
